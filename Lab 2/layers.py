...
class FC(Layer):
  def __init__(self, input_layer, num_outputs, name,
    ...
    self.num_outputs = num_outputs #[NxH]
...
def softmax(x):
    expscores = np.exp(x)
    sumexp = expscores.sum(axis=1).reshape(-1, 1)
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    """
    return -(1.0/x.shape[0])*np.sum(y*np.log(softmax(x)))
  ...
  def backward_inputs(self, x, y):
	...
	"""
    return (1.0/x.shape[0])*(softmax(x) - y)
...
class L2Regularizer():
  ...
  def forward(self):
    ...
    """
    return self.weight_decay*0.5*np.sum(self.weights ** 2)
