...
def one_hot(batch, vocab_size):
    ...
    def _oh(x, vocab_size):
        x_oh = np.zeros((32, vocab_size))
        x_oh[np.arange(32), x] = 1
        return x_oh

    if np.array(batch).ndim == 1:
...
def run_language_model(dataset, max_epochs, seed, hidden_size=100, sequence_size=30, learning_rate=1e-1, sample_every=100):
    vocab_size = len(dataset.sorted_chars)
    simpleRNN = RNN(hidden_size, sequence_size, vocab_size, learning_rate)
    ...
    while current_epoch < max_epochs: 
        ...
        loss, h0 = simpleRNN.step(h0, x_oh, y_oh)
        ...
        if batch % sample_every == 0: 
            ...
            sample_encoded = sample(dataset.encode(seed), 200, simpleRNN)
            print("===============================================================================================")
...
if __name__ == "__main__":
    # Prepare dataset
    dataset = Dataset(input_file="a",sequence_size=30, batch_size=32)
    dataset.preprocess("data/selected_conversations.txt")
    dataset.create_minibatches()
    print(dataset.batches[-1][-1])
    # Start learning
    run_language_model(dataset, 50, sequence_size=dataset.sequence_size, seed="HAN:\nIs that good or bad?\n\n")
